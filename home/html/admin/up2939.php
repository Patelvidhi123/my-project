        ...
        if ($_FILES['image']['error'] == UPLOAD_ERR_OK) {
            ...
            if (in_array($_FILES['image']['type'], $allowedTypes)) {
                ...
                if (move_uploaded_file($imageTmpName, $imagePath)) {
                    ...
                    $query = "UPDATE `t2939` SET name=?, image=?, price=? WHERE id=?";
            ...
            $query = "UPDATE `t2939` SET name=?, price=? WHERE id=?";
            $stmt = mysqli_prepare($cn, $query);
            mysqli_stmt_bind_param($stmt, 'ssi', $name, $price, $id);
        }

        if (mysqli_stmt_execute($stmt)) {
            echo "<script>alert('Toy updated successfully'); window.location.href='2939a.php';</script>";
        } else {
            echo "<script>alert('Error updating toy');</script>";
        }
    }

    $query = "SELECT * FROM `t2939` WHERE id=?";
    $stmt = mysqli_prepare($cn, $query);
...
<body>
    <div class="container">
        ...
        <a class="back-link" href="2939a.php">Back to List</a>
    </div>
</body>
</html>
