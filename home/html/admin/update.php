        ...
        if ($_FILES['image']['error'] == UPLOAD_ERR_OK) {
            ...
            if (in_array($_FILES['image']['type'], $allowedTypes)) {
                ...
                if (move_uploaded_file($imageTmpName, $imagePath)) {
                    ...
                    $query = "UPDATE toys SET name=?, image=?, price=? WHERE id=?";
            ...
            $query = "UPDATE toys SET name=?, price=? WHERE id=?";
            $stmt = mysqli_prepare($cn, $query);
            mysqli_stmt_bind_param($stmt, 'ssi', $name, $price, $id);
        }

        if (mysqli_stmt_execute($stmt)) {
            echo "<script>alert('Toy updated successfully'); window.location.href='0-6a.php';</script>";
        } else {
            echo "<script>alert('Error updating toy');</script>";
        }
    }

    $query = "SELECT * FROM toys WHERE id=?";
    $stmt = mysqli_prepare($cn, $query);
