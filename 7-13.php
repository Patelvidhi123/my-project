...
<head>
    ...
    <title>Toys for 7-13 Years</title>
    <link rel="stylesheet" type="text/css" href="7-13.css">
    <style>
        .out-of-stock {
            color: red;
            font-weight: bold;
...
<div class="container">
    ...
    <div class="toy-grid">
        ...
        $result = mysqli_query($cn, "SELECT * FROM `713`"); // Enclosed table name in backticks
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                echo '<div class="toy-item">';
                echo '<img src="' . $row['image'] . '" alt="' . $row['name'] . '">';
                echo '<p>$' . $row['price'] . '</p>';
                if ($row['stock_status'] == 'Out of Sto') {
